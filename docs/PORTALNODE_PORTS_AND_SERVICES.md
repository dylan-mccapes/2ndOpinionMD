...
| `relation "rag_corpus" already exists` replaying **`05a_*.sql.gz`** | **`05a` is not idempotent.** Use **`./scripts/portalnode4090_replay_05a.sh`** (same **`DUMP_DIR`** / **`PG*`** as restore) — drops **`rag_corpus`**, reapplies 05a stubs + **`05a`**. Or full **`sudo bash scripts/portalnode4090_reset_mkg_target_db.sh`** + **`portalnode4090_restore_mkg.sh`**. After **`05b`**, skip **`01`** if auth seed already loaded. |
