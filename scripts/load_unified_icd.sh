...
REQUIRED_FILES=(
    "database/schemas/unified_icd_schema.sql"
    "ontology_loaders/icd/load_unified_icd10cm.py"
    "ontology_loaders/icd/load_unified_icd11.py"
    "scripts/map_icd_systems.py"
)
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    psql -d knowledgegraph -f database/schemas/unified_icd_schema.sql
else
    sudo cp database/schemas/unified_icd_schema.sql /tmp/ && sudo chmod 644 /tmp/unified_icd_schema.sql
...
python3 ontology_loaders/icd/load_unified_icd10cm.py "$ICD10CM_FILE"
echo "✅ ICD-10-CM loading completed"
echo ""

echo "🌐 Step 3: Loading ICD-11 data..."
python3 ontology_loaders/icd/load_unified_icd11.py "$ICD11_FILE"
echo "✅ ICD-11 loading completed"
echo ""

echo "🔗 Step 4: Generating cross-system mappings..."
python3 scripts/map_icd_systems.py 0.75
