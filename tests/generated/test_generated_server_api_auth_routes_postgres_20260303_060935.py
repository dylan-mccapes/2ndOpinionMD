...
def test_login_for_access_token_bad_credentials(monkeypatch):
    ...
    assert exc.value.detail['code'] == 'bad_credentials'

@pytest.mark.asyncio
def test_login_for_access_token_email_not_verified(monkeypatch):
    class DummyForm:
        username = 'user@example.com'
        password = 'goodpass'
    class DummySession: pass
...
@pytest.mark.asyncio
def test_login_for_mobile_access_token_success(monkeypatch):
    class DummyForm:
        username = 'user@example.com'
        password = 'goodpass'
    class DummySession: pass
    ...
    except auth_routes_postgres.HTTPException:
        pytest.fail('Should not raise for verified user')
...
def test_verify_email_invalid_token(monkeypatch):
    ...
    assert exc.value.status_code == auth_routes_postgres.status.HTTP_400_BAD_REQUEST
    assert 'Invalid verification token' in str(exc.value.detail)
...
def test_forgot_password_email_not_found(monkeypatch):
    ...
    result = None
    import asyncio
    result = asyncio.run(auth_routes_postgres.forgot_password(DummyBody(), mock.Mock(), DummySession()))
...
def test_reset_password_invalid_token(monkeypatch):
    ...
    assert exc.value.status_code == auth_routes_postgres.status.HTTP_400_BAD_REQUEST
    assert 'Invalid reset token' in str(exc.value.detail)
...
def test_resend_verification_already_verified(monkeypatch):
    ...
    assert exc.value.status_code == auth_routes_postgres.status.HTTP_400_BAD_REQUEST
    assert exc.value.detail['code'] == 'already_verified'
...
def test_accept_doctor_invite_wrong_user_type(monkeypatch):
    ...
    assert 'Only patients' in str(exc.value.detail)
...
def test_accept_patient_invite_wrong_user_type(monkeypatch):
    ...
    assert exc.value.status_code == auth_routes_postgres.status.HTTP_403_FORBIDDEN
    assert 'Only doctors' in str(exc.value.detail)
