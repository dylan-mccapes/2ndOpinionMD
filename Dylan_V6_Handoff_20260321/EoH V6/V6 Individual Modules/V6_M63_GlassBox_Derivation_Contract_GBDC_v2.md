...
- DEGRADATION_STATE carriers are module-specific in structure. M63 carries the degradation representation exactly as the producing module emitted it, without normalizing across modules. The carrier's `artifact_pointer` references the producing module's native degradation artifact. No universal degradation schema is imposed by M63.
...
A DerivationChain is replay-deterministic if, given identical inputs, identical module versions, and identical governance state, the chain can be reconstructed with identical structure and pointer topology. Role-context affects which chain elements are visible versus REDACTED in presentation, but does not alter the underlying chain structure. Two replays under different role-contexts MUST produce structurally identical chains with different visibility masks.
...
replay_metadata: {
  ...
  input_sequence_ref: pointer | null
}
```

The `input_sequence_ref` field is required for modules whose outputs are path-dependent (i.e., the current output depends on the ordered history of prior inputs, not just the current input snapshot). For non-path-dependent modules, this field MAY be null.

### 7.3 Rules

- Replay determinism is a structural property of the chain, not a guarantee of clinical outcome equivalence.
- If any module version or governance state has changed since chain production, replay MUST be flagged as STALE — not silently rerun under new conditions.
- Replay operates against stored inputs as they existed at chain production time. If upstream input artifacts have been amended, corrected, or versioned since production, replay still uses the original artifact versions referenced in the chain. Evaluating the system's behavior against current inputs is a new invocation, not a replay, and produces a new DerivationChain with its own `chain_id`.
- For path-dependent modules, if `input_sequence_ref` is absent or the referenced input history is unavailable, replay MUST be flagged as REPLAY_INCOMPLETE rather than silently attempted from a snapshot alone.
