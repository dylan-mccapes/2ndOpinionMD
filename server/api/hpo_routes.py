...
async def get_hpo_term(hpo_id: str, session: AsyncSession = Depends(get_session)):
    try:
        sql = """
        ...
        WHERE hpo_id = :h
        """
        row = (await session.execute(text(sql), {"h": hpo_id})).mappings().first()
        if not row:
