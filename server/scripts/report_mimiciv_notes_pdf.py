...
def load(text_col: str,
    ...
    try:
        ...
        # Mapping coverage from the helper map table
        map_counts = q(conn, """
        ...
        return counts, breakdown, joinab, window, cats, map_counts, mapped_any
    finally:
        conn.close()
...
def main(out="db_integrity_reports/08_mimiciv_notes.pdf", use_ai=False):
    ...
    counts, breakdown, joinab, window, cats, map_counts, mapped_any = load(
    ...
    # Verdict rubric:
    # - Fail: zero rows
    # - Warn: too many non-joinable hadm_id OR null rate is high without good mapped coverage
    # - Pass: otherwise
    if total == 0:
        verdict = "fail"
    elif bad_rate > 0.10:
    ...
    def flow(story, content_width):
        ...
        if source_col: cols_line += f", source: <b>{source_col}</b>"
        if cat_col:    cols_line += f", category: <b>{cat_col}</b>"
    ...
    if use_ai:
        facts = {
            "counts": summary,
            "columns_detected": {
                "text": text_col, "chart": chart_col, "source": source_col, "category": cat_col
            },
            "null_rate": null_rate,
            "joinability": {"hadm_in": hadm_in, "hadm_not_in": hadm_not_in, "bad_rate": bad_rate},
            "mapping": {"mapped_any": mapped_any, "by_method": map_counts},
            "window": window,
            "breakdown_sample": (breakdown[:5] if breakdown else []),
        }
        ai_obj = ai_analyze(
            system=(
                "You are auditing MIMIC-IV-Note integrity. "
