    ...
    async for user_doc in mongo_db.users.find():
        ...
        if len(user_batch) >= batch_size:
            async with SessionLocal() as session:
                ...
                print(f"Committed user batch, processed {user_count} users")
            user_batch.clear()
        ...
        if len(entry_batch) >= batch_size:
            async with SessionLocal() as session:
                for entry in entry_batch:
                    session.add(entry)
                await session.commit()
                print(f"Committed entry batch, processed {entry_count} journal entries")
            entry_batch.clear()
